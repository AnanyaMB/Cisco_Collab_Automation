...
for eachDevicePool in devicePoolInfo:
    ...
    devicePoolDict.update({"devicePoolName":eachDevicePool,
                            ...
                            "unknownDeviceCount":devicePoolCount})
    devicePoolList.append(devicePoolDict)

devicePoolDf = pd.DataFrame(devicePoolList, columns=['devicePoolName', 'totalCount','unknownDeviceCount'])
devicePoolDf.to_excel("PRC-unknown-output.xlsx",index=False)